...
def main(verbose):
    ...
    ### Do button counts. After 1000 button presses, record part 1. Stop when cycles for all 4 portions have been found
    lowTot, highTot = 0, 0
    i = 0

    portionStates = defaultdict(lambda: defaultdict(lambda: 0))
    cyclesFound = {p: None for p in portions}
    while any(c is None for c in cyclesFound.values()) or i < 1000:
