...
fn part1(contents: String) -> i64 {
    let mut sands: Vec<((i64, i64, i64), (i64, i64, i64))> = Vec::from_iter(contents.lines().map(|line| {
        line.split("~").map(|pos| {
            pos.split(",").map(|x| x.parse::<i64>().unwrap()).collect_tuple().unwrap()
...
fn part2(contents: String) -> i64 {
    ...
    let to_disintegrate = HashSet::from_iter(0..sands.len()).difference(&safe).map(|x| *x).collect::<Vec<usize>>();
    let mut count: i64 = 0;
    ...
    for dis_ix in to_disintegrate {
        ...
        while let Some(ix) = queue.pop_front() {
            ...
            for under in underneath.get(&ix).unwrap_or(&HashSet::new()).iter() {
                if on_top_of.get(under).unwrap().iter().all(|d| disintegrated.contains(d)) {
...
fn main() {
    ...
    let relative_path = if root.ends_with("rust_2023_22") {
        RelativePath::new("../../Inputs/2023_22.txt")
    } else {
        RelativePath::new("/Inputs/2023_22.txt")
    };
